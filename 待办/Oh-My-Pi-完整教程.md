...
| 你的情况 | 建议 |
|----------|------|
| 用过 Claude Code/Codex，想换/混用模型 | omp 不绑供应商，模型角色可组合 |
| 需要 Windows 原生支持 | omp 无 WSL 依赖 |
| 团队需要统一配置、自定义工具 | omp 有扩展系统 + Auth Broker |
| 重度依赖 LSP 重构、真实调试 | omp 的 LSP/DAP 集成比同行深 |
| 只是好奇想试试 | `/login` 选供应商，五分钟上手 |
