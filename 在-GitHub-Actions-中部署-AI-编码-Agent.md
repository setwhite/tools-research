...
gh-agent 是一个 GitHub Action，基于 Pi 编码 Agent 构建。放入仓库后，在 Issue 里留言就能召唤 AI Agent 来读代码、改文件、提 PR。无需服务器，所有 commit 以 bot 身份提交。
...
类似项目：[Claude Code Action](https://github.com/anthropics/claude-code-action)（绑定 Anthropic）、[Codex Action](https://github.com/openai/codex-action)（绑定 OpenAI）。[gh-agent](https://github.com/linhan-dev/gh-agent) 是社区项目，不绑定任何服务商。
...
写法和 Pi Agent 本地 skill 完全一样。

## 六、总结

- 部署就是一个 workflow 文件 + 一个 Secret，配合 `AGENTS.md` 和 `.agents/skills/` 即可让 Agent 遵守你的规范
